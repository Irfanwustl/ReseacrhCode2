...
def get_all_tcga_projects():
    ...
    # Use more general filters to fetch all projects first
    params = {
        "filters": json.dumps({
            "op": "in",
            "content": {
...
def get_files_with_case_ids(project, data_type, fields):
    """Query the GDC API to fetch all file metadata with case IDs for a given project and data type."""
    ...
    while page <= total_pages:
        ...
        total_pages = response_data['data']['pagination']['pages']
        page += 1

    print(f"Total files fetched for project {project}: {len(files)}")
    return files
...
def download_files(manifest_path, download_dir):
    ...
    # Use the predefined path to call gdc-client
...
def main(master_directory):
    ...
    for project_id in projects:
        files_data = get_files_with_case_ids(project_id, data_type, fields)
